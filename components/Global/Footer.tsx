			...
			<div className=" bg-pureBlack py-4 my-0">
				<div className="md:container px-4 mx-auto flex flex-col-reverse lg:flex-row items-center justify-between ">
					...
					>
						...
						/>
						<Link
							className="lg:inline-block items-center justify-center mt-2 px-1 text-lightGreyTwo hover:text-primary-default hover:underline"
							...
							globalContext?.copyrightLinks?.map((item: any, keys: number) => (
								<Fragment key={keys}>
									...
									>
										<Link
											href={`${item?.node?.url}`}
											className="text-lightGreyTwo transition-all duration-200 ease-in-out text-sm hover:text-primary-default"
										>
											{item?.node?.label}
										</Link>
									</motion.li>
